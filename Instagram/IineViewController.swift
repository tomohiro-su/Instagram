...
//                             }
                    ...
                    let postRef = Firestore.firestore().collection(Const.IinePath).document()
        
//                             // likesに更新データを書き込む
                             var iname = iineName.text!
                             var iines = iineText.text!
                             let PostRef = Firestore.firestore().collection(Const.NamePath).document(postData.id)
                             postRef.updateData(["iname": updateValue])
